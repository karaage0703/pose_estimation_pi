...
if __name__ == '__main__':
    ...
    try:
        ...
        while True:
            ...
            image_canvas = np.full((h, w, 3), 128)
            image_canvas[(h - new_h)//2:(h - new_h)//2 + new_h,(w - new_w)//2:(w - new_w)//2 + new_w, :] = resized_image


            background_image = cv2.imread('ch_ed.jpg')
            background_image = cv2.resize(background_image, (new_w, new_h), interpolation = cv2.INTER_CUBIC)
            background_canvas = np.full((h, w, 3), 128)
            background_canvas[(h - new_h)//2:(h - new_h)//2 + new_h,(w - new_w)//2:(w - new_w)//2 + new_w, :] = background_image
    
            prepimg = image_canvas.astype(np.float32)
            ...
            for part in range(nPoints):
                ...
                probMap = cv2.resize(probMap, (image_canvas.shape[1], image_canvas.shape[0])) # (256, 256)
            ...
            frameClone = np.uint8(background_canvas.copy())
            for i in range(nPoints):
                for j in range(len(detected_keypoints[i])):
                    cv2.circle(frameClone, detected_keypoints[i][j][0:2], 5, (0, 0, 0), -1, cv2.LINE_AA)
            ...
            for i in range(17):
                for n in range(len(personwiseKeypoints)):
                    ...
                    cv2.line(frameClone, (B[0], A[0]), (B[1], A[1]), (0, 0, 0), 3, cv2.LINE_AA)
